"""Signal E — Domain/Range violation proxy (D241/D242/D245).

Threshold-based on ``grace_extraction_validation_failures_total`` filtered
to ``signal_e.kind_filter`` (default: ``domain_violation``,
``range_violation``). Identical structure to Signal C; the two differ
only in their ``kind_filter`` defaults.

When a future chunk lands ``misplaced_property``, operators add it to
``signal_e.kind_filter`` in YAML — no code changes required (D243-style
extensibility).
"""
...
class SignalEDetector(SignalDetector):
    signal_type: ClassVar[str] = "E"

    async def detect(self, run_context: SignalRunContext) -> list[SignalRecord]:
        cfg = run_context.config.signal_e
        if not cfg.enabled:
            return []
        ...
        if not current.entries:
            ...
            note_prerequisites_not_met(run_context, "E", missing)
            return []
        if not baseline.entries:
            log.info(
        ...
        for entry in current.entries:
            ...
            bucket = per_module.setdefault(
                module, {"max_strength": 0.0, "tuples": []}
            )
        ...
        for module, bucket in per_module.items():
            ...
            grace_metrics.signal_e_strength.set(
